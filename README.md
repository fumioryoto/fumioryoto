# 👋 Hi, I'm Nahid Hasan

🔴 **Cybersecurity Learner | Red Teaming & Penetration Testing Enthusiast**

I’m learning and practicing **Red Teaming and Penetration Testing**, focusing on understanding **how real-world attacks work** and explaining them in a **simple and friendly way**.

---

## 🧠 Focus Areas

- Red Teaming fundamentals & adversary mindset
- Penetration testing methodology
- Web application security testing
- Vulnerability research & CVE analysis
- Linux & Kali Linux
- MITRE ATT&CK & Cyber Kill Chain

---

## 🛠️ Tools & Technologies

**Recon & Enumeration**
- Nmap, Gobuster, Dirsearch

**Web Testing & Exploitation**
- Burp Suite
- SQLmap
- Metasploit

**Post-Exploitation**
- Netcat
- LinPEAS

**Other**
- Git & GitHub
- Linux (Kali, Ubuntu)

---

## 🧪 Hands-on Practice

- TryHackMe & Hack The Box labs
- Vulnerability exploitation walkthroughs
- Proof-of-Concept (PoC) analysis
- Attack flow diagrams for better understanding
- Beginner-friendly notes for students

---

## 🎯 Goals

- Build strong foundations in Red Team operations
- Think like an attacker, act ethically
- Explain complex vulnerabilities in simple terms
- Transition toward advanced Red Team techniques

---

## ⚠️ Ethics & Responsibility

> All activities and content are strictly for **educational purposes**.  
> I perform security testing **only on systems I own or have explicit permission to test**.

---

## 📚 Learning Platforms & Resources

- TryHackMe
- Hack The Box
- OWASP
- MITRE ATT&CK
- CVE databases & security blogs

---

## 📊 GitHub Stats *(optional)*

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=fumioryoto&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=fumioryoto&layout=compact&langs_count=8&theme=tokyonight&hide_border=true"/>
</div>

<div align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=fumioryoto&theme=tokyo-night&hide_border=true&area=true" width="90%"/>
</div>

---

## 📫 Connect With Me

- GitHub  : https://github.com/fumioryoto  
- Facebook: https://facebook.com/fumioryoto
- Telegram: https://t.me/fumioryoto
---

⭐ Always learning. Always improving.
